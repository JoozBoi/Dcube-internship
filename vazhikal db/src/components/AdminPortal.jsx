  ...
  React.useEffect(() => {
    if (activeAdminTab === 'audit') {
      ...
      fetch('/api/admin/audit-logs', {
        ...
        .then(res => {
          if (!res.ok) throw new Error('Status call failed: Admin-only access required.');
          return res.json();
        })
        .then(data => {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8" id="admin-portal-screen">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8" id="admin-columns-split">
        ...
        <div className="lg:col-span-3 space-y-8" id="admin-portal-contents">
          ...
          {/* TAB 3: System Audit & Status Logs */}
          {activeAdminTab === 'audit' && (
            <div className="space-y-6 animate-in fade-in duration-200" id="admin-audit-logs-section">
              <div>
                <h1 className="text-3xl font-black text-gray-900 font-sans tracking-tight">System Audit & Status Logs</h1>
                <p className="text-sm text-gray-400 font-medium font-sans">
                  Security node displaying all low-level creations, updates, and deletions of entities across PostgreSQL.
                </p>
              </div>

              {/* Filtering and search console */}
              <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
                <div className="flex flex-col sm:flex-row gap-4 items-center justify-between mb-4" id="audit-filter-controls">
                  <div className="w-full sm:w-80 relative">
                    <input
                      type="text"
                      placeholder="Search by action, performer, or ID..."
                      value={auditSearchQuery}
                      onChange={(e) => setAuditSearchQuery(e.target.value)}
                      className="w-full pl-4 pr-4 py-2.5 bg-gray-50 border border-gray-150 rounded-xl text-xs sm:text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500 text-gray-800 placeholder-gray-450"
                    />
                  </div>
                  <div className="flex space-x-2 shrink-0">
                    <button
                      onClick={() => {
                        setLoadingAudit(true);
                        fetch('/api/admin/audit-logs', { headers: { 'x-user-role': 'admin' } })
                          .then(res => res.json())
                          .then(data => { setAuditLogsList(data); setLoadingAudit(false); })
                ...
                <div className="border-t border-gray-100 pt-6">
                  {loadingAudit ? (
                    <div className="text-center py-12 text-xs text-gray-400 font-bold font-mono animate-pulse">
                      Executing LIVE REST Status Call to PostgreSQL...
                    </div>
                  ) : auditError ? (
                    ...
                    <div className="space-y-4" id="audit-logs-list-flow">
                      {auditLogsList
                        .filter(log => {
                        ...
                        .map(log => {
                          ...
                          const isUpdate = log.action.includes('UPDATE') || log.action.includes('VOTE');
                          const dateString = new Date(log.timestamp).toLocaleString();

                          return (
                            <div key={log.id} className="border border-gray-100 bg-gray-50/30 p-5 rounded-2xl hover:bg-gray-100/50 transition flex flex-col justify-between gap-3">
                              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-gray-100/70 pb-3">
                                <div className="flex flex-wrap items-center gap-2">
                                  <span className={`px-2.5 py-1 text-[10px] font-black uppercase rounded-lg tracking-wider ${
                                    isCreate ? 'bg-emerald-100 text-emerald-855' :
                                    isDelete ? 'bg-red-100 text-red-855' :
                                    isUpdate ? 'bg-indigo-100 text-indigo-855' : 'bg-amber-100 text-amber-855'
                                  }`}>
                                    {log.action}
                                  </span>
                                  <span className="text-xs font-bold text-gray-300">•</span>
                                  <span className="text-xs font-black text-gray-700">{log.entityType}</span>
                                  <span className="text-[10px] font-mono bg-white border border-gray-100 px-2 py-0.5 rounded text-gray-500">
                                    ID: {log.entityId}
                                  </span>
                                </div>
                                <span className="text-[10px] text-gray-400 font-mono font-bold">{dateString}</span>
                              </div>

                              <div className="space-y-2 pt-1">
                                <div className="flex items-center space-x-2">
                                  <span className="text-xs text-gray-400 font-semibold">Initiator UID/Role:</span>
                                  <span className="text-xs font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md">
                                    {log.performedBy}
                                  </span>
                                </div>
                                {log.details && (
                                  <div className="w-full block bg-white border border-gray-100 p-3 rounded-xl overflow-x-auto text-[11px] font-mono text-gray-500 max-w-full">
                                    {log.details}
                                  </div>
                                )}
                              </div>
                      ...
                      {auditLogsList.filter(log => {
                        ...
                        <div className="text-center py-12 text-xs text-gray-400 font-bold font-sans">
                          No audit status records match the active query.
                        </div>
                      )}
                    </div>
                  )}
