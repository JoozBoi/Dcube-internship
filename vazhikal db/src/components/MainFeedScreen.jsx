...
export const MainFeedScreen = ({
  ...
  const [profileName, setProfileName] = useState(() => loggedInUser ? (loggedInUser.username || 'Sara Wanderer') : 'Sara Wanderer');
  const [profileEmail, setProfileEmail] = useState(() => loggedInUser ? loggedInUser.email : 'sara@example.com');
  const [profilePassword, setProfilePassword] = useState(() => loggedInUser ? (loggedInUser.password || 'secret123') : 'secret123');
  const [profileBio, setProfileBio] = useState(() => loggedInUser ? (loggedInUser.bio || 'Lover of mountain passes, remote temple hikes, and authentic experiences.') : 'Lover of mountain passes, remote temple hikes, and authentic experiences.');

  // Synchronize profile inputs when loggedInUser updates or logins
  React.useEffect(() => {
    if (loggedInUser) {
      setProfileName(loggedInUser.username || 'Sara Wanderer');
      setProfileEmail(loggedInUser.email || 'sara@example.com');
      setProfilePassword(loggedInUser.password || 'secret123');
      setProfileBio(loggedInUser.bio || 'Lover of mountain passes, remote temple hikes, and authentic experiences.');
    } else {
      setProfileName('Sara Wanderer');
      setProfileEmail('sara@example.com');
      setProfilePassword('secret123');
      setProfileBio('Lover of mountain passes, remote temple hikes, and authentic experiences.');
    }
  }, [loggedInUser]);
