...
export default function App() {
  ...
  const [users, setUsers] = useState([
    { username: 'SaraWanderer', email: 'sara@example.com', password: 'secret123', role: 'traveller', bio: 'Lover of mountain passes, remote temple hikes, and authentic experiences.' },
    { username: 'admin', email: 'admin@vazhikal.io', password: '99238382', role: 'admin', bio: 'Secure administrative intelligence node control center.' },
    { username: 'EverestTrek', email: 'info@everesttrekkers.np', password: 'agency123', role: 'agency', companyName: 'Everest Trekkers Co.', bio: 'Licensed Mountain Passes & Sherpa Trail specialists.' },
  ]);

  // Load registered users from database on startup
  React.useEffect(() => {
  ...
  // Load real data from PostgreSQL on mount and on currentRole changes
  React.useEffect(() => {
    const loadAllDatabaseRecords = async () => {
      try {
        ...
        const verifRes = await fetch('/api/verifications', { headers });
        if (verifRes.ok) {
        ...
        const flaggedRes = await fetch('/api/flagged-posts', { headers });
        if (flaggedRes.ok) {
          const flaggedData = await flaggedRes.json();
          setFlaggedPosts(flaggedData);
        }

        // 5. Fetch Comment Reports
        const reportsRes = await fetch('/api/comment-reports', { headers });
        if (reportsRes.ok) {
          const reportsData = await reportsRes.json();
          setCommentReports(reportsData);
        }
      } catch (err) {
  ...
  const handleVotePost = (id, direction) => {
    setPosts(prev => prev.map(p => {
      ...
      // Persist to Postgres database via endpoint
  ...
  // 3. Discussion comments management (Supports Infinite Recursive Nesting)
  const handleAddComment = (postId, text, replyToCommentId) => {
    const commentId = 'comm_' + Date.now();
    const commentAuthor = loggedInUser ? loggedInUser.username : (currentRole === 'admin' ? 'System Administrator' : currentRole === 'agency' ? 'Host Agent' : 'Sojourn Explorer');
    const newC = {
      id: commentId,
  ...
  const handleRemoveFlaggedPost = (id) => {
    ...
    if (incident) {
      ...
      if (pToDelete) {
        fetch(`/api/posts/${pToDelete.id}`, { method: 'DELETE', headers: { 'x-user-role': currentRole } }).catch(err => console.error(err));
      }
    }
  ...
  const handleUpdateProfile = async (updatedFields) => {
    ...
    // Keep memory user lists synced
    setUsers(prev => prev.map(u => 
      u.email === loggedInUser.email || u.username === loggedInUser.username
        ? { ...u, ...updatedFields }
        : u
    ));
